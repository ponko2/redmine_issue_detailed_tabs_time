  ...
  module IssuesHelperPatch
    ...
    module InstanceMethods
      ...
      def get_selected_issue_history_tab(tabs)
        tabs_contain_content = tabs.map { |tab| tab[:name] }

        if valid_tabs.include?(params[:tab]) && tabs_contain_content.include?(params[:tab])
          selected_tab = params[:tab]
        end
        selected_tab ||= valid_tabs.each.find { |v| tabs_contain_content.include? v }
      end
